...
Apps are defined by a YAML [schema](#app-schema). An "app launcher" (such as Terra) parses the app YAML file, and invokes a helm chart to launch the app on a Kubernetes cluster. Once the app is launched, it may be accessed in a browser via a proxy URL.
